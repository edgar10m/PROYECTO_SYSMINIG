...
type
   Tftsmain = class( TForm )
    ...
    mnuMantenimientoParametros: TdxNavBarItem;
    // ----
    ...
    procedure mnuMantenimientoParametrosClick(Sender: TObject);
    procedure gFabricaPbasTecClick(Sender: TObject);
   private
   ...
   public
      { Public declarations }
      //cierra_ventana: integer;
      procedure cambia_version (v:string);    //alk para la version
      procedure arregla_cascada();  // para organizar ventanas en cascada   ALK
   end;
...
uses
   ...
   ptsadminctrusu, ptscaducidad, parbol, ptsrecibe, ptsinventario, fptpar,
...
begin
   ...
   //ALK para evitar el error de que no encuentra la ventana activa
   if (screen.ActiveForm <> nil) and (screen.ActiveForm.Name = 'farbol') then begin
...
begin
   ...
   //cierra_ventana:=0;   // para error al cerrar ventanas al mismo tiempo de cerrar el form
...
begin
   ...
   if gral.bPubVentanaActiva( 'Recepción de Componentes' ) then
      Exit;
...
var
   i: Integer;
begin
{   for i := 0 to MDIChildCount - 1 do    // cambio por que tronaba ALK
      MDIChildren[ i ].Close;      }
   for i := MDIChildCount - 1 downto 0 do         //alk
            MDIChildren[ i ].Close;
...
begin
   ...
   PR_CATALOG( mnuParametros.Caption, 'select ' +
      'clave       vk__Clave_de_Parametro_ast, ' +
   ...
   //fcatalog.foco('SELE_CLAVE_DE_PARAMETRO');
...
begin
   ...
   PR_CATALOG( mnuUsuarios.Caption, 'select ' +
      'cuser    vk__Clave_de_Usuario_ast, ' +
      'nombre   v___Nombre_ast, ' +
      'paterno  v___Apellido_Paterno, ' +
      'materno  v___Apellido_Materno, ' +
      'password n___Password_ast, ' +
      'correo v___Correo ' +
      'from tsuser ' +
...
begin
   ...
   PR_CATALOG( mnuCapacidades.Caption, 'select ' +
      ...
      'ccapacidad       vkc_Clave_de_Capacidad_ast, ' +
      'crol             vkc_Rol, ' +
      'cuser            vkc_Usuario ' +
      'from tscapacidad ' +
   ...
   dm.feed_combo( fcatalog.pan.FindComponent( 'SELE_CLAVE_DE_CAPACIDAD' ) as tcombobox,                 
      'select unique ccapacidad from tscapacidad where crol =' + g_q + g_rol + g_q + 'order by 1' );      //ALK
      //'select unique ccapacidad from tscapacidad where crol =' + g_q + 'GENERAL' + g_q + 'order by 1' );        //RGM
...
begin
   ...
   PR_CATALOG( mnuClases.Caption, 'select ' +
      'cclase             vk__Clase_ast, ' +
      'tipo               v_c_Tipo_ast, ' +
      'descripcion        v___Descripcion, ' +
      'analizador         v_c_Herramienta_Analisis, ' +
   ...
   dm.feed_combo( fcatalog.pan.FindComponent( 'SELE_HERRAMIENTA_ANALISIS' ) as tcombobox,
...
begin
   ...
   PR_CATALOG( mnuSistemas.Caption, 'select ' +
      'csistema              vk__Sistema_ast, ' +
      'coficina              v_c_Oficina_ast, ' +
      'descripcion           v___Descripcion, ' +
   ...
   try
      ...
      if dm.sqlselect( dm.q1, 'select * from tsuser where cuser=' + g_q + txtusuario.text + g_q ) then begin
         ...
         if dm.sqlselect( dm.q2, 'select * from tsroluser where cuser=' + g_q + txtusuario.text + g_q ) then begin
            ...
            g_rol:= dm.q2.fieldbyname( 'crol' ).AsString;
...
begin
   ...
   if dm.capacidad( 'MENU IZQUIERDO VALIDACIONES ESTATICAS' ) then
      ...
      gEstaticas.Visible := false;
   if dm.capacidad( 'MENU IZQUIERDO CODIGO MUERTO' ) then
      mnuMuerto.Visible := true
   else
      mnuMuerto.Visible := false;
...
begin
   ...
   if gral.bPubVentanaActiva('Conversión') then
      Exit;
...
begin
   ...
   try
      ...
      if ierror = 0 then begin
         ...
         fmListaDependencias.sin_controles(0);
   ...
   if Application.MessageBox( pchar( '  ¿Desea salir del sistema? ' ), 'Confirmar', MB_ICONQUESTION or MB_YESNO ) = IDNO then begin
      ...
      try
         ...
         try
            ...
            showMessage('Ocurrio un error al cerrar las ventanas activas.');
         end;
      end;
   end;
end;
...
begin
   if gral.bPubVentanaActiva( 'Cambios Masivos Automáticos' ) then
      Exit;
...
begin
   if gral.bPubVentanaActiva( 'Validaciones Estáticas' ) then
      Exit;
...
begin
   if gral.bPubVentanaActiva( 'Código Muerto' ) then
      Exit;
...
begin
   if gral.bPubVentanaActiva( 'Genera Productos' ) then
      Exit;
...
procedure Tftsmain.mnuMantenimientoParametrosClick(Sender: TObject);
begin
   PR_MANTPARAM;
end;

procedure Tftsmain.gFabricaPbasTecClick(Sender: TObject);
begin
   //iHelpContext := IDH_TOPIC_T01900;
end;

procedure Tftsmain.arregla_cascada();  // para organizar ventanas en cascada   ALK
begin
   mnuCascadaClick(self);
end;
