...
type
   Tdm = class( TDataModule )
    ...
    spEjecutaActivos: TADOStoredProc;
    SaveDialog1: TSaveDialog; //JCR201211
   ...
   public
      ...
      function cuenta_registros(query:string):integer;
      procedure exporta_texto( titulo,query,query_resumen: String);
      procedure exporta_texto_GlbBlockAtributos(sTitulo,sParclase,sParbib,sParprog:string);
   end;

var

   g_version: string = '2017051700'; //'2016102000';//'2016072901'; //'2016072011'; // '2016050401';//'2015082400';//'2015081900';
   ...
   g_usuario: string;
   g_rol: String;       // alk para tener el rol
   g_empresa: string;
   ...
   g_sistema:String;    // ALK para tener el sistema
   g_arbol_activo: integer = 0;  // para saber si el arbol ya se solicito   ALK
implementation
uses
   //isvsserver1,
   ptscomun, ptsmain,
...
begin
   ...
   if dm.sqlselect( dm.q1, 'select * from user_tables where table_name=' + g_q + 'TS_DETA_COMPLEJ' + g_q) = FALSE then begin
    ...
    actualiza_version('2016102000',ver,
      ...
      ' PRIMARY KEY (REGLA))');
   actualiza_version( '2017051700', ver, 'alter table tsrela add ('+
      'XBIB VARCHAR(1500))');
   ...
   try
      ...
      on E: exception do begin
         Application.MessageBox( pchar( 'ERROR SQL: ' + sele + ' - ' + E.Message ),
            pchar( 'Mensaje de SQLSELECT' ), MB_OK );
         sqlselect := false;
      end;
   end;
end;
...
begin
   ...
   for i := 0 to g_borrar.Count - 1 do begin
      try
         if deletefile( g_borrar[ i ] ) then
            borra_lista.Add(g_borrar[ i ]);
      except
      end;
   end;
   borra_lista.SaveToFile(g_tmpdir+'/ArchivosBorradosSysMining.txt');
   borra_lista.Free;
end;
...
begin
   ...
   if not bRepetido then begin //no existe, no repetido
      ...
      //finally
         ...
         qConsulta.Free;
         showmessage('Rebasa recursos del sistema');
         abort;
      end;
      qconsulta.Free;
   end
...
function Tdm.cuenta_registros(query:string):integer;
var qq:TADOquery;
begin
   qq:=TADOquery.Create(self);
   qq.ConnectionString:=ADOConnection1.ConnectionString;
   try
      if dm.sqlselect(qq,query) then
         cuenta_registros:=qq.RecordCount
         //cuenta_registros:=qq.Fields[0].AsInteger
      else begin
         showmessage('Error en el query:'+query);
         cuenta_registros:=0;
      end;
   except
      showmessage('El SQL no concluyó correctamente, probablemente la información no esté completa');
      cuenta_registros:=0;
   end;
   qq.Free;
end;
procedure Tdm.exporta_texto( titulo,query,query_resumen: String);
var qq:TADOquery;
  F: TextFile;
  i,previo,contador:integer;
  x,anterior,actual,archivo,sufijo,extension:string;
begin
   ...
   archivo:=copy(savedialog1.FileName,1,length(savedialog1.FileName)-length(extension));
   sufijo:='';
   contador:=0;
   qq:=TADOquery.Create(self);
   qq.ConnectionString:=dm.ADOConnection1.ConnectionString;
   if dm.sqlselect(qq,query) then begin
      AssignFile( F, archivo+sufijo+extension);
      Rewrite( F );
      writeln(f,titulo);
      x:='"'+qq.fields[6].asstring+
         ' '+qq.fields[5].asstring+
         ' '+qq.fields[4].asstring+'",';
      previo:=0;
      while not qq.eof do begin
         inc(contador);
         if contador mod 100000 =0 then begin
            i:=(contador div 100000);
            sufijo:=format('%.2d',[i]);
            closefile(f);
            AssignFile( F, archivo+sufijo+extension);
            Rewrite( F );
         end;
         actual:=qq.fields[0].asstring+' '+
            qq.fields[1].asstring+' '+
            qq.fields[2].asstring+' '+
            qq.fields[3].asstring+' '+
            qq.fields[4].asstring+' '+
            qq.fields[5].asstring+' '+
            qq.fields[6].asstring;
      ...
      writeln(f,x);
      if dm.sqlselect(qq,query_resumen) then begin
         writeln(f,'RESUMEN');
         while not qq.eof do begin
            x:='"'+qq.fields[2].asstring+
               ' '+qq.fields[1].asstring+
               ' '+qq.fields[0].asstring+
               '",'+qq.Fields[3].AsString;
            writeln(f,x);
            qq.Next;
         end;
...
procedure Tdm.exporta_texto_GlbBlockAtributos(sTitulo,sParclase,sParbib,sParprog:string);
var
  F: TextFile;
  i,j,previo:integer;
  x:string;
begin
   savedialog1.DefaultExt:='csv';
   savedialog1.Filter:='Texto separado por comas (*.csv)|*.csv';
   savedialog1.FileName:=stringreplace(sTitulo,' ','_',[rfreplaceall])+'_'+sParclase+'_'+sParbib+'_'+sParprog+'.csv';
   if savedialog1.Execute=false then exit;
   if fileexists(savedialog1.FileName) then begin
      if application.MessageBox('El archivo ya existe, desea reemplazarlo?','Confirme',MB_YESNO)=IDNO then exit;
   end;
   ...
   Rewrite( F );
   writeln(f,sTitulo+' '+sParclase+' '+sParbib+' '+sParprog);
   previo:=aGlbBlockAtributos[0].Renglon;
   for i := 0 to length( aGlbBlockAtributos ) - 1 do begin
      if aGlbBlockAtributos[i].TipoBlock<>'FlowActionBlock' then continue;
      if aGlbBlockAtributos[i].Renglon>previo then begin
         writeln(f,x);
         x:='';
         for j:=1 to (aGlbBlockAtributos[i].Columna div 120) do x:=x+',';
      end;
      x:=x+'"'+aGlbBlockAtributos[i].Clase+
         ' '+aGlbBlockAtributos[i].Biblioteca+
         ' '+aGlbBlockAtributos[i].Programa+'",';
      previo:=aGlbBlockAtributos[i].Renglon;
   end;
   writeln(f,x);
   closefile(f);
   if FileExists(savedialog1.FileName) then
