...
implementation
uses
   ptsdm, ptsMain, ufmDocumentacion, ptsgral; //Math;
...
var
   ...
   begin
      ...
      if not dm.sqlinsert( sInsert ) then begin
         Application.MessageBox( pchar( dm.xlng( 'ERROR... no puede insertar en tsdocumento' ) ),
            pchar( dm.xlng( 'Agregar ' ) ), MB_OK );
         Exit;
      end;
   ...
   begin
      ...
      if not dm.sqlinsert( sInsert ) then begin
         Application.MessageBox( pchar( dm.xlng( 'ERROR... no puede insertar en tsdocrevision' ) ),
            pchar( dm.xlng( 'Agregar ' ) ), MB_OK );
         Exit;
      end;
...
begin
   ...
   if bModificado then begin
      ...
      if bPriNuevo then begin
         ...
         //sNOMBRE := ExtractFileName( sNOMBRE );
         //sNOMBRE := Trim( StringReplace( sNOMBRE, sEXTENSION, '', [ ] ) );
         ...
         {if dm.sqlselect( dm.q1,
            ...
            'WHERE UPPER( NOMBRE ) = UPPER(' + g_q + sNOMBRE + g_q + ')'+
            ' AND CPROG = ' + g_q + sCPROG + g_q +
            ' AND CBIB = ' + g_q + sCBIB + g_q +
            ' AND CCLASE = ' +g_q + sCCLASE + g_q ) then begin}

         if dm.bPubDocumentoExiste( sNOMBRE, sCPROG, sCBIB, sCCLASE ) then begin
            Application.MessageBox( Pchar( 'Nombre de documento existente.' + chr( 13 )+
               'Registre uno diferente.' ), Pchar( 'Aviso' ), MB_OK );
            Exit;
         end;

         if not bInsertarTSDOCUMENTO then
            Exit;

         //TSDOCREVISION
         sUSUARIO_REV := g_usuario;

         if not bInsertarTSDOCREVISION then
            Exit;
         ...
         dm.bInsertarTSDOCBLOB( iIDDOCTO, iIDREVISION, iTAMNORMAL, iTAMCRC, sPriNombre );

         with ftsMain do
            for i := 0 to MDIChildCount - 1 do
               if MDIChildren[ i ].ClassName = 'TfmDocumentacion' then begin
                  ( MDIChildren[ i ] as TfmDocumentacion ).bPubPoblarTabla;
               end;

         CanClose := True;
      end
      else begin //NO nuevo
         SRichViewEdit1.RichViewEdit.SaveRTF( g_tmpdir + '\' + sPriNombre, False );

         iIDDOCTO := iPriIDDocto;
         sUSUARIO_REV := g_usuario;

         bInsertarTSDOCREVISION;

         //TSDOCBLOB
         iTAMNORMAL := 0;
         iTAMCRC := 0;

         dm.bInsertarTSDOCBLOB(
            iIDDOCTO, iIDREVISION, iTAMNORMAL, iTAMCRC, g_tmpdir + '\' + sPriNombre );
...
begin
   ...
   rvActionFonts1.Caption := rvActionFonts1.Caption + ' (Standard)';
   rvActionFontEx1.Caption := rvActionFontEx1.Caption + ' (Advanced)';
