...
type
  PFNHtmlHelpA = ^TFNHtmlHelpA;
  TFNHtmlHelpA = function(hwndCaller: HWND; pszFile: PAnsiChar;
    uCommand: UINT; dwData: DWORD): HWND; stdcall;

  PFNHtmlHelpW = ^TFNHtmlHelpW;
  TFNHtmlHelpW = function(hwndCaller: HWND; pszFile: PWideChar;
    uCommand: UINT; dwData: DWORD): HWND; stdcall;
...
begin
  ...
  begin
    ...
    if HelpModule <> 0 then
      try
        @HH := GetProcAddress(HelpModule, 'HtmlHelpA');
      except
        @HH := nil;
      end;
  end;
