...
type
   TfmSVSDiagrama = class( TForm )
      ...
      mnuExportarVSD: TdxBarButton;
      mnuExportarPDF: TdxBarButton;
    TabSheet2: TGroupBox;
    ...
    cxGrid1Level1: TcxGridLevel;
      ...
      procedure mnuExportarVSDClick( Sender: TObject );
...
implementation
uses
   ptsdm, ptsgral, uDiagramaRutinas, ClipBrd, ptsmain;
...
begin
   ...
   sNombreArchivo := sGlbExportarDiagramaDialogo( exImgWMF, atDiagrama, sCaption );
   ...
   if FileExists( sNombreArchivo ) then begin
      Application.MessageBox(
         Pchar( 'Existe un documento con el mismo nombre en la ruta especificada.' + chr( 13 ) +
         'Elija uno diferente para continuar con la exportación.' ), 'Aviso', MB_OK );
      Exit;
   end;

   Screen.Cursor := crSqlWait;
   try
      GlbExportarDgr_A_WMF( atDiagrama, sNombreArchivo );
   finally
      Screen.Cursor := crDefault;
   end;
end;
...
begin
   ...
   if sClassName = 'TDIAGRAMARC' then
      with ( dcControl as TDiagramArc ) do
         if ( SourceLinkPoint.AnchorIndex < 0 ) or
            ( TargetLinkPoint.AnchorIndex < 0 ) then
            atDiagrama.Undo;

   if sClassName = 'TDIAGRAMBEZIER' then
      with ( dcControl as TDiagramBezier ) do
         if ( SourceLinkPoint.AnchorIndex < 0 ) or
            ( TargetLinkPoint.AnchorIndex < 0 ) then
            atDiagrama.Undo;

   if sClassName = 'TDIAGRAMPOLYLINE' then
      with ( dcControl as TDiagramPolyLine ) do
         if ( SourceLinkPoint.AnchorIndex < 0 ) or
            ( TargetLinkPoint.AnchorIndex < 0 ) then
            atDiagrama.Undo;
end;
...
procedure TfmSVSDiagrama.mnuTextoBuscarEnter( Sender: TObject );
begin
   mnuBuscarAnterior.Enabled := False;
   mnuBuscarSiguiente.Enabled := False;
end;

procedure TfmSVSDiagrama.FormShow( Sender: TObject );
begin
   if FormStyle = fsMDIChild then
      if gral.iPubVentanasActivas > 0 then
         gral.PubExpandeMenuVentanas( True );
...
procedure TfmSVSDiagrama.mnuExportarVSDClick( Sender: TObject );
var
   sNombreArchWMF: String;
   sNombreArchivo, sCaption: string;
   sel : integer;
begin
   sCaption := Caption;
   ...
   sNombreArchivo := sGlbExportarDiagramaDialogo( exVisio, atDiagrama, sCaption );
   ...
   Screen.Cursor := crSqlWait;
   try
      sNombreArchWMF := g_tmpdir + '\' + sCaption + '.wmf';
      //primero exportamos a WMF
      GlbExportarDgr_A_WMF( atDiagrama, sNombreArchWMF );

      if not FileExists( sNombreArchWMF ) then begin
         Application.MessageBox( 'No se puede exportar a Visio.', 'Aviso', MB_OK );
         Exit;
      end;

      try
         //exportar a VSD
         GlbExportarDgr_A_VSD( sNombreArchWMF, sNombreArchivo );
      finally
         if FileExists( sNombreArchWMF ) then
            DeleteFile( sNombreArchWMF );
      end;
   finally
      Screen.Cursor := crDefault;
   end;
...
procedure TfmSVSDiagrama.mnuExportarPDFClick( Sender: TObject );
var
   sNombreArchWMF: String;
   sNombreArchivo, sCaption: string;
begin
   ...
   sNombreArchivo := sGlbExportarDiagramaDialogo( exPDF, atDiagrama, sCaption );
   ...
   try
      ...
      if not FileExists( sNombreArchWMF ) then begin
         Application.MessageBox( 'No se puede exportar a PDF.', 'Aviso', MB_OK );
         Exit;
      end;

      try
         //exportar a PDF
         //GlbExportarDgr_A_PDF( sNombreArchWMF, sNombreArchivo );
         dm.ExportAsPdf( sNombreArchWMF, sNombreArchivo );

         if dm.sqlselect( dm.q1, 'select dato from parametro where clave=' + g_q + 'MUESTRA_DOC' + g_q ) then begin
            if uppercase(dm.q1.fieldbyname( 'dato' ).AsString) = 'SI' then
               if FileExists(sNombreArchivo) then
                  ShellExecute( 0, 'open', pchar( sNombreArchivo ), nil, PChar( ExtractFilePath(sNombreArchivo)), SW_SHOW );
         end;
