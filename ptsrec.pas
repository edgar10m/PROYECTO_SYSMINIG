...
function recibeclick(compos: Tstrings; origen: string;
   ...
   procedure reemplaza_alternos(w_xcclase, w_xbib,pcclase,pcbib,pcprog:string;
      var hcclase,hcbib:string; hcprog,w_sistema_hijo,polimorfismo:string);  //<RGM20170424>
   procedure implementa_herencia(clase,biblioteca,programa,pcclase,pcbib,pcprog,
      hcclase,hcbib,hcprog,auxiliar,coment:string);
   procedure provee_herencia(clase,biblioteca,programa:string);
   function es_java(clase:string):boolean;
...
procedure reemplaza_alternos(w_xcclase, w_xbib,pcclase,pcbib,pcprog:string;
   var hcclase,hcbib:string; hcprog,w_sistema_hijo,polimorfismo:string);  //<RGM20170424>
   // <RGM20170424> Esta rutina reemplazará las sustituciones de SCRATCH, ya probada se eliminará el código antiguo
   // <RGM20170424> Aquí se agregará el parámetro para clases hijas alternas
var
    bib,aux_consulta:string;
    where_clase,where_bib,where_xcclase,where_xbib,where_sistema:string;
    i,k:integer;
    lista_xbib:Tstringlist;
    lista_xcclase:Tstringlist;
    function mismo_cbib:boolean;
    begin
      while dm.q1.Eof=false do begin
         if dm.q1.FieldByName('cbib').AsString=pcbib then begin
            hcclase:=dm.q1.fieldbyname('cclase').asstring;
            hcbib:=dm.q1.fieldbyname('cbib').asstring;
            mismo_cbib:=true;
            exit;
         end;
         dm.q1.Next;
    ...
    function mismo_hcbib:boolean;
    var
      cuenta_pol1,cuenta_pol2:integer;
      aux:String;
    begin
      cuenta_pol1:=cuenta_caracteres(polimorfismo,'_');
      while dm.q1.Eof=false do begin
         cuenta_pol2:=cuenta_caracteres(dm.q1.FieldByName('POLIMORFISMO').AsString,'_');

         if (dm.q1.FieldByName('hcbib').AsString=pcbib) and
            (cuenta_pol1=cuenta_pol2) then begin
            hcclase:=dm.q1.fieldbyname('hcclase').asstring;
            hcbib:=dm.q1.fieldbyname('hcbib').asstring;
            mismo_hcbib:=true;
            if polimorfismo=dm.q1.FieldByName('POLIMORFISMO').AsString then
               exit;
         end;
         dm.q1.Next;
      end;
...
begin
   if (trim(w_xcclase)='') and (trim(w_xbib)='') then exit;    // entra sólo si hay alternos
   if (hcclase<>'XXX') and (pos('SCRATCH',hcbib)=0) then exit; // si no trae XXX o SCRATCH en biblioteca se sale

   lista_xcclase:=Tstringlist.Create;   // prepara clase
   if hcclase='XXX' then begin
      where_clase:='';
      if trim(w_xcclase)='' then
         lista_xcclase.Add('')
      else
         lista_xcclase.CommaText:=w_xcclase;
   end
   else begin
      where_clase:=' and hcclase='+g_q+hcclase+g_q+' ';
      lista_xcclase.Add('');
   end;

   lista_xbib:=Tstringlist.Create;      // prepara biblioteca
   bib:=stringreplace(hcbib,'_SCRATCH_','%',[rfreplaceall]);
   bib:=stringreplace(bib,'SCRATCH_','%',[rfreplaceall]);
   bib:=stringreplace(bib,'_SCRATCH','%',[rfreplaceall]);
   bib:=stringreplace(bib,'SCRATCH','%',[rfreplaceall]);
   if pos('%',bib)>0 then begin
      if bib='%' then
         where_bib:=''
      else
         where_bib:=' and hcbib like '+g_q+bib+g_q+' ';
      if trim(w_xbib)='' then
         lista_xbib.Add('')
      else
         lista_xbib.CommaText:=w_xbib;
   end
   else begin
      where_bib:=' and hcbib='+g_q+bib+g_q+' ';
      lista_xbib.Add('');
   end;

   if b_otros_sistemas then
      where_sistema:=''
   else
      where_sistema:='and sistema='+g_q+w_sistema_hijo+g_q+' ';
   // busca en TSPROG
   for k:=0 to lista_xcclase.Count-1 do begin            // Reemplaza con alternos
      if lista_xcclase[k]='' then
         where_xcclase:=''                                             // clase='...' xc=any
      else
         where_xcclase:=' and cclase='+g_q+lista_xcclase[k]+g_q+' ';   // clase='XXX' xc='......'
      for i:=0 to lista_xbib.Count-1 do begin
         if trim(lista_xbib[i])='' then
            where_xbib:=''                                             // bib='....' o bib='..%..' xb=''
         else
            where_xbib:=' and cbib='+g_q+lista_xbib[i]+g_q+' ';        // bib='..%..' xb='....'
         if dm.sqlselect(dm.q1,'select * from tsprog '+
            ' where cprog=' + g_q + hcprog + g_q +
            stringreplace(stringreplace(where_bib,'hcbib=','cbib=',[rfreplaceall]),
               'hcbib like','cbib like',[rfreplaceall])+
            where_xbib+
            stringreplace(where_clase,'hcclase=','cclase=',[rfreplaceall])+
            where_xcclase+
            where_sistema) then begin
            lista_xcclase.Free;
            lista_xbib.Free;
            if dm.q1.RecordCount = 1 then begin
               ...
               if mismo_cbib then exit;       // si hay más de un registro, selecciona el que tenga la misma biblioteca del padre
            exit;
         end;
      end;
   end;
   if es_java(pcclase)=false then begin    // si es java se obliga a buscar sólo en las bibliotecas alternas
      if dm.sqlselect(dm.q1,'select * from tsprog '+  // Reemplaza normal
         ' where cprog=' + g_q + hcprog + g_q +
         stringreplace(stringreplace(where_bib,'hcbib=','cbib=',[rfreplaceall]),
            'hcbib like','cbib like',[rfreplaceall])+
         stringreplace(where_clase,'hcclase=','cclase=',[rfreplaceall])+
   ...
   for k:=0 to lista_xcclase.Count-1 do begin            // Reemplaza con alternos
      if lista_xcclase[k]='' then
         ...
         where_xcclase:=' and hcclase='+g_q+lista_xcclase[k]+g_q+' ';   // clase='XXX' xc='......'
      for i:=0 to lista_xbib.Count-1 do begin
         if trim(lista_xbib[i])='' then
            where_xbib:=''                                              // bib='....' o bib='..%..' xb=''
         else
            where_xbib:=' and hcbib like '+g_q+'%'+lista_xbib[i]+g_q+' ';        // bib='..%..' xb='....'

         aux_consulta:='select distinct hcbib,hcclase,polimorfismo from tsrela '+
                  ' where hcprog=' + g_q + hcprog + g_q +
                  where_bib+
                  where_xbib+
                  where_clase+
                  where_xcclase+
                  ' and hcbib not like '+g_q+'%SCRATCH%'+g_q+' '+
                  where_sistema;
         if dm.sqlselect(dm.q1,aux_consulta) then begin
            lista_xcclase.Free;
            lista_xbib.Free;
            if dm.q1.RecordCount = 1 then begin
               hcclase:=dm.q1.fieldbyname('hcclase').asstring;
               ...
               if mismo_hcbib then exit;       // si hay más de un registro, selecciona el que tenga la misma biblioteca del padre
            exit;
         end;
      end;
   end;
   if es_java(pcclase)=false then begin    // si es java se obliga a buscar sólo en las bibliotecas alternas
      if dm.sqlselect(dm.q1,'select distinct hcbib,hcclase,polimorfismo from tsrela '+  // Reemplaza normal
         ' where hcprog=' + g_q + hcprog + g_q +
         where_bib+
         where_clase+
         ' and hcbib not like '+g_q+'%SCRATCH%'+g_q+' '+
...
begin
   ...
   dm.sqldelete('delete tsrela '+
      ...
      ' and icprog='+g_q+hcprog+g_q+
      ' and icbib='+g_q+hcbib+g_q+
      ' and icclase='+g_q+hcclase+g_q);
   dm.sqldelete('delete tsrela '+
      ' where ocprog='+g_q+programa+g_q+
      ' and ocbib='+g_q+biblioteca+g_q+
      ' and occlase='+g_q+clase+g_q+
      ' and pcbib='+g_q+pcbib+'.'+pcprog+g_q+
      ' and pcclase='+g_q+'MTD'+g_q+
      ' and icprog='+g_q+hcprog+g_q+
      ' and icbib='+g_q+hcbib+g_q+
      ' and icclase='+g_q+hcclase+g_q);
   if dm.sqlselect(dm.q1,'select * from tsrela '+
      ...
      ' and hcclase='+g_q+'MTD'+g_q+
      ' and hcbib=pcbib||'+g_q+'.'+g_q+'||pcprog'+
      ' order by hcclase,hcbib,hcprog') then begin
      while not dm.q1.Eof do begin
         dm.sqlinsert('insert into tsrela (ocprog,ocbib,occlase,'+
            'pcprog,pcbib,pcclase,'+
            'hcprog,hcbib,hcclase,'+
            'sistema,ambito,'+
            'icprog,icbib,icclase,'+
            'polimorfismo,hsistema,hparametros,hinterfase,auxiliar,'+
            'orden) '+
            ' values('+g_q+programa+g_q+','+g_q+biblioteca+g_q+','+g_q+clase+g_q+','+
            g_q+pcprog+g_q+','+g_q+pcbib+g_q+','+g_q+pcclase+g_q+','+
            g_q+dm.q1.fieldbyname('hcprog').AsString+g_q+','+
            g_q+pcbib+'.'+pcprog+g_q+','+
            ...
            g_q+hcprog+g_q+','+g_q+hcbib+g_q+','+g_q+hcclase+g_q+','+
            ...
            g_q+auxiliar+g_q+','+
            g_q+'8'+copy(dm.q1.fieldbyname('orden').asstring,2,3)+g_q+')');
         dm.sqlinsert('insert into tsrela (ocprog,ocbib,occlase,'+
            'pcprog,pcbib,pcclase,'+
            'hcprog,hcbib,hcclase,'+
            'sistema,ambito,'+
            ...
            'orden) '+
            ' values('+g_q+programa+g_q+','+g_q+biblioteca+g_q+','+g_q+clase+g_q+','+
            g_q+dm.q1.fieldbyname('hcprog').AsString+g_q+','+           //pcprog
            g_q+pcbib+'.'+pcprog+g_q+','+                               //pcbib
            g_q+dm.q1.fieldbyname('hcclase').AsString+g_q+','+          //pcclase
            g_q+dm.q1.fieldbyname('hcprog').AsString+g_q+','+           //hcprog
            g_q+dm.q1.fieldbyname('hcbib').AsString+g_q+','+            //hcbib
            g_q+dm.q1.fieldbyname('hcclase').AsString+g_q+','+          //hcclase
            g_q+dm.q1.fieldbyname('sistema').AsString+g_q+','+
            g_q+dm.q1.fieldbyname('ambito').AsString+g_q+','+
            g_q+hcprog+g_q+','+g_q+hcbib+g_q+','+g_q+hcclase+g_q+','+
            g_q+dm.q1.fieldbyname('polimorfismo').AsString+g_q+','+
            ...
            g_q+dm.q1.fieldbyname('hparametros').AsString+g_q+','+
            g_q+dm.q1.fieldbyname('hinterfase').AsString+g_q+','+
            g_q+dm.q1.fieldbyname('auxiliar').AsString+g_q+','+
            g_q+'9'+copy(dm.q1.fieldbyname('orden').asstring,2,3)+g_q+')');
         dm.q1.Next;
      end;
   end;
end;
procedure provee_herencia(clase,biblioteca,programa:string);
begin
   if clase<>'JAV' then exit;
   // busca las clases definidas en el paquete, si no tiene hijos no necesita aplicar herencia
   if dm.sqlselect(dm.q3,'select distinct pcprog,pcbib,pcclase from tsrela '+
      ' where ocprog='+g_q+programa+g_q+
      ' and ocbib='+g_q+biblioteca+g_q+
      ' and occlase='+g_q+clase+g_q+
      ' and pcclase='+g_q+'JLA'+g_q+
      ' order by  pcbib,pcprog') then begin
      while not dm.q3.Eof do begin
         // busca las clases que le están heredando
         if dm.sqlselect(dm.q4,'select * from tsrela '+
            ' where hcprog='+g_q+dm.q3.fieldbyname('pcprog').AsString+g_q+
            ' and hcbib='+g_q+dm.q3.fieldbyname('pcbib').AsString+g_q+
            ' and hcclase='+g_q+dm.q3.fieldbyname('pcclase').AsString+g_q+
            ' and coment='+g_q+'EXTENDS'+g_q) then begin
            while not dm.q4.Eof do begin
               implementa_herencia(
                  dm.q4.fieldbyname('occlase').AsString,
                  dm.q4.fieldbyname('ocbib').AsString,
                  dm.q4.fieldbyname('ocprog').AsString,
                  dm.q4.fieldbyname('pcclase').AsString,
                  dm.q4.fieldbyname('pcbib').AsString,
                  dm.q4.fieldbyname('pcprog').AsString,
                  dm.q4.fieldbyname('hcclase').AsString,
                  dm.q4.fieldbyname('hcbib').AsString,
                  dm.q4.fieldbyname('hcprog').AsString,
                  dm.q4.fieldbyname('auxiliar').AsString,
                  dm.q4.fieldbyname('coment').AsString);
               dm.q4.Next;
            end;
         end;
         dm.q3.Next;
            ...
            if dm.sqlselect(dm.q1,'select * from tsrela '+
               ...
               while not dm.q1.Eof do begin
                  ...
                  sele := 'insert into tsrela (pcprog,pcbib,pcclase,hcprog,hcbib,hcclase,modo,' +
                     ...
                     'polimorfismo,xcclase,hsistema,hparametros,hinterfase,xbib) ' +
                     ' values(' +
                     ...
                     g_q + dm.q1.fieldbyname('hparametros').AsString + g_q + ',' +
                     g_q + dm.q1.fieldbyname('hinterfase').AsString + g_q + ','+
                     g_q + dm.q1.fieldbyname('xbib').AsString + g_q + ')';
                  dm.sqlinsert(sele);
                  b_ok:=true;
                  dm.q1.Next;
               end;
...
var
   ...
   ucla: string;
   w_lineainicio, w_lineafinal, w_ambito,w_auxiliar, w_icprog, w_icbib, w_icclase, w_polimorfismo, q_polimorfismo, w_xcclase, w_xbib, w_sistema_hijo, w_parametros, w_interfase: string;
   w_where: string;
   wb,wc:Tstringlist;
   w_ok:boolean;
   tempo: string;
...
begin
   ...
   for i := 0 to lista.Count - 1 do begin
      ...
      while pos('][', tempo) > 0 do begin
         ...
         else if copy(tempo, 1, 3) = 'AU=' then
            w_auxiliar := copy(tempo, 4, pos('][', tempo) - 4)
      ...
      if owner_tipo='CS' then
         m[5] := stringreplace(m[5], '\', '.', [rfreplaceall]); // Csharp
      nclase:=m[3];
      nbib:=m[4];
      reemplaza_alternos(w_xcclase,w_xbib,m[0],m[1],m[2],nclase,nbib,m[5],w_sistema_hijo,w_polimorfismo);  //<RGM20170424>
      m[3]:=nclase;
      m[4]:=nbib;
      implementa_herencia(owner_tipo,biblioteca,owner_prg,m[0],m[1],m[2],m[3],m[4],m[5],w_auxiliar,m[10]);
      ...
      sele := 'insert into tsrela (pcprog,pcbib,pcclase,hcprog,hcbib,hcclase,modo,' +
         'organizacion,externo,coment,orden,sistema,ocprog,ocbib,occlase,lineainicio,lineafinal,ambito,auxiliar,icprog,icbib,icclase,' +
         'polimorfismo,xcclase,hsistema,hparametros,hinterfase,xbib) ' +
         ' values(' +
         g_q + programa + g_q + ',' +
         g_q + bibli + g_q + ',' +
   ...
   if b_tiene_var then
      reemplaza_var_en_hijos(owner_tipo,biblioteca,owner_prg); //<RGM20170521> probable error
      //reemplaza_var_en_hijos(clase,biblioteca,programa);
   provee_herencia(owner_tipo,biblioteca,owner_prg);
   m.Free;
   wb.free;
   wc.Free;
