...
//    procedure SalirDelMenu(Sender: TObject);
   private
    ...
    tt_existentes,tt_identificados,tt_faltantes,tt_faltantesW1,tt_sin_uso,tt_activos:string;
    tg:array of Tgroup;
    b_impresion:boolean;
...
var descripcion,nquery,nselect,nselect1,nwhere,nwhere1,b1,nanexo,nanexo1,nanexo2,nanexo3:string;
    x1,x2,c1:integer;
    b2:Tstringlist;
begin
...
////jcr         nselect:='select cclase clase,cbib libreria,cprog componente';
...
////jcr         nwhere:=' where cclase='+g_q+tipo+g_q;
...
////jcr            nselect:='select hcclase clase,hcbib libreria,hcprog componente ';
            nselect:='select x.hcclase clase,x.hcbib libreria,x.hcprog componente,'+
                     't.lineas_blanco,t.lineas_total,t.lineas_comentario,t.lineas_efectivas';
            nwhere:=' where x.hcclase='+g_q+tipo+g_q+' and sistema='+g_q+sistema+g_q;
////jcr            nwhere:=' where hcclase='+g_q+tipo+g_q+' and sistema='+g_q+sistema+g_q;
         if  tab.Tabs[tab.TabIndex]='FALTANTES' then
             nwhere:=' where hcclase='+g_q+tipo+g_q+' and sistema='+g_q+sistema+g_q+
                     ' and ((hcprog,hcbib,hcclase) not in (select cprog,cbib,cclase from tsprog)))';
...
////jcr   nquery:=nselect+copy(nquery,x1,x2-x1)+nwhere+' order by 3,2';

   nquery:=nselect+copy(nquery,x1,x2-x1)+nanexo1+nwhere+
           nanexo2+g_q+sistema+g_q+nanexo+' UNION ALL '+
           nselect1+copy(nquery,x1,x2-x1)+' x '+nwhere+
           nanexo2+g_q+sistema+g_q+nanexo3+' order by 3,2 ';
...
//      ' where hcclase in (select cclase from tsclase where tipo='+g_q+'ANALIZABLE'+g_q+')'+
   ...
   tt_faltantes:='select sistema,hcclase,count(*) total from '+
      '(select distinct sistema,hcclase from tsrela '+
...
begin
   ...
   arch:=g_tmpdir+'\inventario'+Wfecha+'.html';
   g_borrar.Add(arch);
   arch:=g_tmpdir+'\inventarioIMP'+Wfecha+'.html';
   g_borrar.Add(arch);
   close;
end;
...
//      vaux2:=vaux2+1;
   ...
   x.Add('</HTML>');
   x.Add('</HTML>');
   x.savetofile(g_tmpdir+'\Inventario'+Wfecha+'.html');
   x1.savetofile(g_tmpdir+'\InventarioIMP'+Wfecha+'.html');
   x.free;
   x1.free
...
begin
   screen.Cursor:=crsqlwait;
   creaweb;
   web.Navigate(g_tmpdir+'\Inventario'+Wfecha+'.html');
   screen.Cursor:=crdefault;
end;
...
begin
   b_impresion:=true;
   Web.Navigate(g_tmpdir+'\InventarioIMP'+Wfecha+'.html');
